...
export default class AlbumResource extends Resource {
    listAll() {
        return this.get('albums').then(res => res.data);
    }

    remove(id) {
        return this.delete(`albums/${id}`).then(res => res.data.msg);
    }

    findById(id) {
        return this.get(`albums/${id}`).then(res => res.data);
    }
    ...
    save(album) {
        ...
        if(album.id) {
            ...
            return axios.post(`${this.baseUrl}albums/${album.id}`, body, options).then(res => res.data);
        } else
            return axios.post(this.baseUrl + 'albums', body, options).then(res => res.data);
    }
